...
and because it censors endings, where resolutions, apologies, and
final-scene harms live, it can only undercount early-era harm and
acknowledgment.
...
318 cases (83.9\%), which is 61.0\% of all persons in the dataset.
...
those who still qualify has grown no milder, during the \#MeToo-era
seasons measurably worse (see the companion).
...
majority of false accusations in the first place. Institutional error is
rendered frequent but gentle; civilian malice is rendered rare but
lethal. That allocation protects the heroes twice over, and it is
exactly the pattern the copaganda literature would predict a
police-centered drama to produce (Kappeler \& Potter, 2018;
Karakatsanis, 2025).
...
Six limitations bound these findings. First, and most important, the
...
that era); the early-era harm and apology counts are therefore best read
as undercounts. Four episodes had no transcript and contribute only
ambiguity flags; one had encoding damage and contributes a single
lower-confidence record; season 27 was three episodes old at collection.
...
Fourth, the codebook's precision-first design excludes ambiguous cases,
so 521 is best read as a floor. Fifth, the model leaked small amounts of
...
the figure of roughly 513. Sixth, the dataset codes no demographics.
...
None of these limitations, I think, threatens the central proportions:
...
finding, and the apology and exposure results come from controlled
fields with almost no missing data.
...
by the detectives themselves, whose inference starts most false
accusations and whose disclosure habits accompany the worst damage. And
the ledger closes without acknowledgment 92.1\% of the time.
...
Orden, T. (in preparation). Fewer accusations, worse outcomes: The
retreat of the false accusation plot in Law \& Order: SVU, 1999-2026.
Manuscript.
